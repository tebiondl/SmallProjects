...
def reset_values():
    ...
    global maquinas_operativas, maquinas_reserva
    # Establecer parámetros iniciales
    maquinas_operativas = 10
    maquinas_reserva = 4
    tiempo_actual = 0
...
# Función para gestionar los fallos de las máquinas
def rutina_llegada(tsuceso, id):
    ...
    global num_clientes_op1, num_clientes_op2, num_clientes_op3, maquinas_operativas, maquinas_reserva, tiempo_reparacion_op1, tiempo_reparacion_op2
    global tiempo_reparacion_op3, eventos_fallos, tiempo_sim_start,simultaneo, tiempo_funcionamiento_sistema_start, tiempo_funcionamiento_sistema
    tiempo_actual = tsuceso
    num_clientes += 1
    num_llegadas += 1
    
    #Si hay maquinas en reserva, usarlas
    if maquinas_operativas > 0:
        ...
        if maquinas_operativas == 9:
            ...
            tiempo_funcionamiento_sistema += (tiempo_actual - tiempo_funcionamiento_sistema_start)
    
    #Registrar el fallo
    eventos_fallos.append({
        "nombre": "Fallo " + str(num_llegadas) + " en máquina"+str(id),
        "tiempo": tsuceso
    })
    ...
    if num_clientes_op1 == 0 and num_clientes > 0:
        num_clientes_op1 += 1
        num_clientes -= 1
        #Si los 3 trabajan a la vez, nos lo apuntamos
        if num_clientes_op2 > 0 and num_clientes_op3 > 0 and simultaneo == False:
            tiempo_sim_start = tiempo_actual
            simultaneo = True
        reparar = tiempo_hasta_reparar(tasa_reparacion)
        #Asignar siguiente reparación por parte del operario 1 y luego registrarlo
        LISTA["tiempo_salida_1"] = tiempo_actual + reparar
        tiempo_relativo_reparaciones.append({
                "nombre": "Rep " + str(num_llegadas + 1) + " en operario 1 para la máquina "+str(id),
    ...
    if num_clientes_op2 == 0 and num_clientes > 0:
        ...
        num_clientes -= 1
        #Si los 3 trabajan a la vez, nos lo apuntamos
        if num_clientes_op1 > 0 and num_clientes_op3 > 0 and simultaneo == False:
            tiempo_sim_start = tiempo_actual
            simultaneo = True
        reparar = tiempo_hasta_reparar(tasa_reparacion)
        #Asignar siguiente reparación por parte del operario 2 y luego registrarlo
    ...
    if num_clientes_op3 == 0 and num_clientes > 0:
        ...
        num_clientes -= 1
        #Si los 3 trabajan a la vez, nos lo apuntamos
        if num_clientes_op1 > 0 and num_clientes_op2 > 0 and simultaneo == False:
            tiempo_sim_start = tiempo_actual
            simultaneo = True
        reparar = tiempo_hasta_reparar(tasa_reparacion)
        #Asignar siguiente reparación por parte del operario 3 y luego registrarlo
...
#Funcion que gestiona las reparaciones del operario 1
def rutina_servidor_1(tsuceso):
    ...
    #Si ya hay 10 máquinas funcionando, añadir la máquina nueva a la reserva, en otro caso, ponerla a funcionar
    ...
    #Si después de reparar, hay alguna máquina esperando a ser reparada, ponerse a repararla
    if num_clientes > 0:
        num_clientes_op1 += 1
        num_clientes -= 1
        reparar = tiempo_hasta_reparar(tasa_reparacion)
...
#Funcion que gestiona las reparaciones del operario 2
...
#Funcion que gestiona las reparaciones del operario 3
...
def start_sim():
    ...
    global tasa_reparacion, tiempo_funcionamiento_sistema
    ...
    #Se comprueba que al menos un valor de LISTA sea diferente a -1 para seguir el bucle
    while any(value != -1 for value in LISTA.values()):
        #Se filtran los -1 de la lista para no tenerlos en cuenta
        numeros_filtrados = {k: v for k, v in LISTA.items() if v != -1}
        #Se busca el suceso más cercano para avanzar hacia ese instante en el tiempo
        ...
        #Se va aumentando la tasa de reparación
...
def print_histogram():
    tiempos_fallos = [fallo["tiempo_relativo"] for fallo in tiempo_relativo_fallos]
    
    # Crea el histograma de fallos
    plt.subplot(2, 2, 1)
    plt.hist(tiempos_fallos, bins=100, edgecolor='black')
    plt.xlabel('Tiempo de fallo')
    plt.ylabel('Cantidad de fallos')
    plt.title('Tiempos de fallos')

    # Extrae los tiempos de las reparaciones
    tiempos_reparaciones = [reparacion["tiempo_relativo"] for reparacion in tiempo_relativo_reparaciones]

    # Crea el histograma de reparaciones
    plt.subplot(2, 2, 2)
    plt.hist(tiempos_reparaciones, bins=100, edgecolor='black')
    plt.xlabel('Tiempo de reparación')
    plt.ylabel('Cantidad de reparaciones')
    plt.title('Tiempos de reparaciones')
    
    # Crea el histograma de fallos acumulado
    plt.subplot(2, 2, 3)
    plt.hist(tiempos_fallos, bins=100, edgecolor='black', cumulative=True)
    plt.xlabel('Tiempo de fallo')
    plt.ylabel('Cantidad de fallos acumulados')
    plt.title('Tiempos de fallos acumulados')

    # Crea el histograma de reparaciones acumulado
    plt.subplot(2, 2, 4)
    plt.hist(tiempos_reparaciones, bins=100, edgecolor='black', cumulative=True)
    plt.xlabel('Tiempo de reparación')
    plt.ylabel('Cantidad de reparaciones acumulados')
    plt.title('Tiempos de reparaciones acumulados')

    # Ajusta el diseño y muestra los histogramas
    plt.tight_layout()
    plt.show()
    
def print_histogram_2():
    pass

#Para crea un excel con datos
def print_excel():
    global eventos_fallos, eventos_reparaciones, tiempo_relativo_fallos, tiempo_relativo_reparaciones
    
    if len(eventos_fallos) > len(eventos_reparaciones):
        # Truncar lista1 para que tenga la misma longitud que lista2
        eventos_fallos = eventos_fallos[:len(eventos_reparaciones)]
    elif len(eventos_reparaciones) > len(eventos_fallos):
        # Truncar lista2 para que tenga la misma longitud que lista1
        eventos_reparaciones = eventos_reparaciones[:len(eventos_fallos)]
    
    if len(tiempo_relativo_fallos) > len(tiempo_relativo_reparaciones):
        # Truncar lista1 para que tenga la misma longitud que lista2
        tiempo_relativo_fallos = tiempo_relativo_fallos[:len(tiempo_relativo_reparaciones)]
    elif len(tiempo_relativo_reparaciones) > len(tiempo_relativo_fallos):
        # Truncar lista2 para que tenga la misma longitud que lista1
        tiempo_relativo_reparaciones = tiempo_relativo_reparaciones[:len(tiempo_relativo_fallos)]
